...
export function NotificationsPage() {
  ...
  return (
    <div className="space-y-4">
      <h2 className="font-display text-xl font-bold text-navy">Notifications</h2>
        ...
        {isLoading ? (
          ...
          data.map((n) => {
            ...
            return (
              <div
                key={n.id}
                className={`bg-white rounded border p-4 flex items-start justify-between gap-4 ${
                  n.read_at ? "border-black/8 opacity-60" : "border-navy/20"
                }`}
              >
                <div>
                ...
                <div className="flex gap-2 shrink-0">
                  {link && (
                    <Link to={link} className="text-xs text-navy hover:text-orange font-medium">
                      View →
                    </Link>
                  )}
                  {!n.read_at && (
                    <button
                      ...
                      className="text-xs text-black/40 hover:text-navy"
                    >
                      Mark read
                    </button>
                  )}
